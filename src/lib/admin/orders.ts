...
export async function createOrder(orderData: {
  ...
  if (isSupabaseConnected()) {
    ...
    // Build insert data, handling nullable fields for pickup orders
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    // Only include hotel_id and room_number if they have values
    // (avoids NOT NULL / UUID parse errors for pickup orders)
    if (orderData.hotel_id) insertData.hotel_id = orderData.hotel_id;
    if (orderData.room_number) insertData.room_number = orderData.room_number;
    ...
    if (orderError) throw new Error(`${orderError.message} (code: ${orderError.code})`);
    ...
    if (itemsError) throw new Error(`${itemsError.message} (code: ${itemsError.code})`);
    ...
    if (fetchError) throw new Error(`${fetchError.message} (code: ${fetchError.code})`);
