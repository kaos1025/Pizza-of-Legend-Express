...
### 메뉴 구조
- 피자: 15종, **L(14인치) 단일 사이즈** (R 사이즈 제거 — 2026-06 클라이언트 피드백)
  - `pizza_size` ENUM 은 유지하되 코드 레벨에서 R 필터 / 신규 가격은 L 만 운영
- 반반 피자: 15종 중 자유 조합, 조합 상관없이 **고정가 L ₩29,900**
- 세트 메뉴: 비활성 (데이터는 보존, UI/주문 제외)
- 사이드(스파게티/치킨 등): 비활성 (업주가 데이터 제거 — 빈 카테고리 탭 숨김)
- 음료: 6종, 소스/추가: 5종 (소스·토핑 add-on 은 유지)

### 영업시간 (Business Hours)
- 기본 영업시간: **매일 12:00–24:00** (Asia/Seoul 기준)
- `store_settings` 의 `business_hours` 키(JSONB)에 저장: `{enabled, open, close, timezone, manual_closed}`
- 영업시간 외 / 임시휴무(manual_closed) 시 주문 차단은 **DB 트리거(check_business_hours, BEFORE INSERT on orders)** 가 최종 enforce
  - 트리거가 `STORE_CLOSED` / `STORE_CLOSED_MANUAL` 로 reject → 주문 API 는 409 + code 반환
- 고객 화면: 영업시간 배지 상시 노출 + 마감 시 상단 배너(오픈 카운트다운) — 브라우징은 허용, 담기/주문만 비활성
- 마감으로 주문 reject 시 **장바구니 보존** (비우지 말 것)
- Admin: `/admin/settings/business-hours` 에서 영업시간/차단 사용/임시휴무 편집
